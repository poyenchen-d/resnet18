...
def train_model():
    ...
    if load_existing_model:
        model.load_state_dict(torch.load("model_weights11.pth"))
    ...
    for epoch in range(epochs):
        ...
        for images, true_boxes, _ in tqdm(dataloader, desc=f"Epoch [{epoch + 1}/{epochs}]"):
            ...
            true_boxes = true_boxes.to(device)  # 将真实框移动到
            optimizer.zero_grad()
        ...
        torch.save(model.state_dict(), "model_weights11.pth")
        print(f"Epoch [{epoch + 1}/{epochs}], Box Loss: {running_loss / len(dataloader):.8f}")

    print("Training complete. Model weights saved to 'model_weights11.pth'.")
